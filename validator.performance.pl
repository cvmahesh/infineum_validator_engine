...
def check_null_values(json_data, table_columns, table_data):
    """Checks if required fields contain null (empty) values."""
    fields = json_data["fields"]
    column_index_map = {col: idx for idx, col in enumerate(table_columns)}
    ...
    for row_num, row in enumerate(table_data, start=1):
        for column, config in fields.items():
            if config.get("required", False) and column in column_index_map:
                ...
                if value == "":  # Checking for blank/null values
                    null_violations.append((row_num, column))
    
    if null_violations:
        log(f" Found {len(null_violations)} blank value violations.")
        for row_num, column in null_violations:
            log(f"   Row {row_num}: Field '{column}' is required but blank.")
    else:
        log(" No blank value violations found.")

    return null_violations


def check_zero_values(json_data, table_columns, table_data):
    """Checks if numeric fields with 'zero_check' enabled contain zero values."""
    fields = json_data["fields"]
    column_index_map = {col: idx for idx, col in enumerate(table_columns)}
    ...
    for row_num, row in enumerate(table_data, start=1):
        for column, config in fields.items():
            if config.get("zero_check", False) and column in column_index_map:
                col_index = column_index_map[column]
                value = row[col_index].strip() if col_index < len(row) else ""

                if value == "0":  # Checking if the value is exactly zero
                    zero_violations.append((row_num, column))

    if zero_violations:
        log(f" Found {len(zero_violations)} zero value violations.")
        for row_num, column in zero_violations:
            log(f"   Row {row_num}: Field '{column}' should not contain zero.")
    else:
        log(" No zero value violations found.")
...
if column_result["is_valid"]:
    ...
    null_value_issues = check_null_values(json_data, table_columns, table_data)  # NEW METHOD
    zero_value_issues = check_zero_values(json_data, table_columns, table_data)  # NEW METHOD
    ...
    log(f" Blank (Null) Value Violations: {len(null_value_issues)}")   
    ...
    if null_value_issues:  # NEW
        log_error(f"Blank (Null) Value Violations: {len(null_value_issues)}")
    if zero_value_issues:  # NEW
